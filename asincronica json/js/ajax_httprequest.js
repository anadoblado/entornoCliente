...
//console.log(xhr.responseText);
    if (xhr.status >= 200 && xhr.status < 300) {
      ...
      json.forEach((el) => {
        ...
        // poner los datos del json que queremos ver
        //$li.innerHTML = `${el.HOMBRE} -- ${el.EDAD}-- ${el.SECCION} `;
        $li.innerHTML = `${el.COD} -- ${el.Nombre} `;
  ...
  //xhr.open("GET", "assets/poveda.json");
  xhr.open("GET", "http://servicios.ine.es/wstempus/js/es/DATOS_TABLA/37808?tip=AM");
